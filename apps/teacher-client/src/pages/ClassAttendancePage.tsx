...
import { Camera, Trash2, Upload, ArrowLeft, Users, CheckCircle, XCircle, AlertCircle } from "lucide-react";
...
export function ClassAttendancePage() {
  ...
  const handleSubmitAttendance = async () => {
    ...
    try {
      ...
      const sessionResp = await fetch(`${API_BASE_URL}/api/attendance/session`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
        body: JSON.stringify({
          class_id: classId,
          teacher_id: "teacher-001", // For demo
      ...
      if (!sessionResp.ok) {
        ...
        throw new Error(error.error || `HTTP error! status: ${sessionResp.status}`);
      }
      ...
      toast.success(`Asistencia registrada: ${result.present_count} presentes, ${result.absent_count} ausentes`);
    } catch (error) {
      console.error("Error submitting attendance:", error);
      toast.error(error instanceof Error ? error.message : "Error al registrar la asistencia");
    } finally {
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-muted-foreground">
              {classData.class.classroom.name} · Período {classData.class.period} · {classData.total} estudiantes
            </p>
          </div>
          {DEBUG_ENABLED && (
        ...
        <div className="space-y-6">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Sesión registrada el {new Date(attendanceResult.timestamp).toLocaleString('es-CL')}
              </CardDescription>
            </CardHeader>
            <CardContent>
              {(() => {
                // Categorize students by confidence
                const confirmed = attendanceResult.present_students.filter(s => s.confidence >= CONFIDENCE_THRESHOLDS.HIGH);
                const likely = attendanceResult.present_students.filter(s => s.confidence >= CONFIDENCE_THRESHOLDS.MEDIUM && s.confidence < CONFIDENCE_THRESHOLDS.HIGH);
                const needsReview = attendanceResult.present_students.filter(s => s.confidence < CONFIDENCE_THRESHOLDS.MEDIUM);
                      ...
                      <Card>
                        <CardContent className="pt-6">
                          <div className="text-center">
                            ...
                            <div className="text-2xl font-bold">{attendanceResult.expected_students}</div>
                            <div className="text-sm text-muted-foreground">Esperados</div>
                          </div>
                        </CardContent>
                      </Card>
                      <Card className="bg-green-50">
                        <CardContent className="pt-6">
                          <div className="text-center">
                            ...
                            <div className="text-xs text-green-600 mt-1">≥95% confianza</div>
                          </div>
                        </CardContent>
                      </Card>
                      <Card className="bg-amber-50">
                        <CardContent className="pt-6">
                          <div className="text-center">
                            ...
                            <div className="text-2xl font-bold text-amber-700">{likely.length + needsReview.length}</div>
                            <div className="text-sm text-amber-700">Por Revisar</div>
                            <div className="text-xs text-amber-600 mt-1">&lt;95% confianza</div>
                          </div>
                        </CardContent>
                      </Card>
                      <Card className="bg-red-50">
                        <CardContent className="pt-6">
                          <div className="text-center">
                            ...
                            <div className="text-2xl font-bold text-red-700">{attendanceResult.absent_count}</div>
                    ...
                    <div className="grid gap-6 md:grid-cols-2">
                        ...
                        <div>
                          ...
                          <div className="space-y-2">
                            {confirmed.map((student) => (
                              <div key={student.student_id} className="p-3 bg-green-50 rounded-lg border border-green-200">
                      ...
                      {likely.length > 0 && (
                        <div>
                          ...
                          <div className="space-y-2">
                            {likely.map((student) => (
                              <div key={student.student_id} className="p-3 bg-amber-50 rounded-lg border border-amber-200">
                                <div className="font-medium">{student.name}</div>
                                <div className="text-sm text-muted-foreground">
                                  RUT: {student.identification}
                                </div>
                                <div className="text-xs text-amber-700 mt-1">
                                  ⚠ {student.confidence.toFixed(1)}% confianza - Revisar
                                </div>
                              </div>
                            ))}
                          </div>
                        ...
                        <div>
                          ...
                          <div className="space-y-2">
                            {needsReview.map((student) => (
                              <div key={student.student_id} className="p-3 bg-orange-50 rounded-lg border border-orange-300">
                                <div className="font-medium">{student.name}</div>
                                <div className="text-sm text-muted-foreground">
                                  RUT: {student.identification}
                                </div>
                                <div className="text-xs text-orange-700 mt-1 font-medium">
                                  ⚠ {student.confidence.toFixed(1)}% confianza - Verificar manualmente
                                </div>
                              </div>
                            ))}
                          </div>
              ...
              <div className="mt-6 pt-6 border-t">
                <Button 
                  onClick={() => {
                    ...
                    submittedPhotos.forEach(photo => URL.revokeObjectURL(photo.preview));
                    setSubmittedPhotos([]);
          ...
          {DEBUG_ENABLED && attendanceResult.debug_info && attendanceResult.debug_info.length > 0 && (
            <Card>
              <CardHeader>
              ...
              <CardContent>
                ...
                <div className="mb-6 p-4 bg-blue-50 border border-blue-200 rounded-lg">
                  <h4 className="font-semibold text-blue-900 mb-2">Resumen de Detección</h4>
                  <div className="grid gap-2 text-sm">
                    <div className="flex justify-between">
                      <span className="text-blue-700">Total de rostros detectados:</span>
                      <span className="font-medium text-blue-900">{attendanceResult.total_faces_detected}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-blue-700">Estudiantes identificados:</span>
                      <span className="font-medium text-blue-900">{attendanceResult.present_count}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-blue-700">Rostros sin identificar:</span>
                      <span className="font-medium text-blue-900">
                        {attendanceResult.total_faces_detected - attendanceResult.present_count}
                      </span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-blue-700">Umbral de confianza:</span>
                  ...
                  {attendanceResult.total_faces_detected > attendanceResult.present_count && (
                    <div className="mt-3 pt-3 border-t border-blue-300">
                      <p className="text-xs text-blue-800">
                        <strong>💡 Posibles razones por las que algunos rostros no se identificaron:</strong>
                      </p>
                      <ul className="text-xs text-blue-700 mt-1 ml-4 list-disc space-y-1">
                        <li>El estudiante no tiene fotos registradas en el sistema</li>
                        <li>La similitud está por debajo del umbral del 95%</li>
                        <li>La foto es de baja calidad o el rostro está parcialmente oculto</li>
                        <li>El ángulo o iluminación es muy diferente a las fotos registradas</li>
                        <li>El estudiante no está inscrito en esta clase</li>
                      </ul>
                      <div className="mt-2 p-2 bg-blue-100 rounded text-blue-900 text-xs">
                        <strong>✨ Ahora analizando todos los rostros:</strong> El sistema analiza cada rostro detectado individualmente para identificar a todos los estudiantes en la foto.
                      </div>
                    </div>
                  )}
                </div>
                ...
                <div className="space-y-6">
                  {attendanceResult.debug_info.map((photoInfo) => (
                    <div key={photoInfo.photoIndex} className="border rounded-lg p-4 space-y-3">
                      <div className="flex items-center justify-between">
                      ...
                      {submittedPhotos[photoInfo.photoIndex] && (
                        <div className="relative inline-block max-w-full">
                          <img
                            src={submittedPhotos[photoInfo.photoIndex].preview}
                      ...
                      <div className="space-y-4 mt-4">
                        {/* Matched faces first */}
                        {photoInfo.faces.filter(f => f.matchedStudent).length > 0 && (
                          <div>
                            <h5 className="text-sm font-semibold text-green-700 mb-2 flex items-center gap-1">
                              <CheckCircle className="h-4 w-4" />
                              Identificados ({photoInfo.faces.filter(f => f.matchedStudent).length})
                            </h5>
                            <div className="grid gap-2">
                              {photoInfo.faces.filter(f => f.matchedStudent).map((face, idx) => (
                                <div
                                  key={idx}
                                  ...
                                  <div className="flex items-start justify-between">
                                    <div className="flex-1">
                                      ...
                                      <div className="text-sm text-green-700 mt-1">
                                        Confianza detección: {face.confidence.toFixed(1)}%
                                        {' · '}
                                        Similitud: <strong>{face.matchedStudent!.similarity.toFixed(1)}%</strong>
                                      </div>
                                      
                                      {/* Show other potential matches */}
                                      {face.topMatches && face.topMatches.length > 1 && (
                                        <div className="mt-2 text-xs">
                                          <div className="font-semibold text-gray-600 mb-1">
                                            Otras posibles coincidencias:
                                          </div>
                                          <div className="space-y-1">
                                            {face.topMatches.slice(1).map((match, i) => (
                                              <div 
                                                key={i} 
                        ...
                        {photoInfo.faces.filter(f => !f.matchedStudent).length > 0 && (
                          <div>
                            <h5 className="text-sm font-semibold text-red-700 mb-2 flex items-center gap-1">
                              ...
                              No Identificados ({photoInfo.faces.filter(f => !f.matchedStudent).length})
                            </h5>
                            <div className="grid gap-2">
                              {photoInfo.faces.filter(f => !f.matchedStudent).map((face, idx) => (
                                <div
                                    ...
                                    <div className="flex-1">
                                      ...
                                      {face.noMatchReason && (
                                        <div className="text-xs text-amber-800 mt-2 p-2 bg-amber-100 rounded border border-amber-300">
                                          <strong>⚠ Razón:</strong> {face.noMatchReason}
                                        </div>
                                      )}
                                      
                                      {/* Show top matches if any */}
                                      {face.topMatches && face.topMatches.length > 0 && (
                                        <div className="mt-2 text-xs">
                                          <div className="font-semibold text-orange-700 mb-1">
                                            💡 Mejores coincidencias encontradas:
                                          </div>
                                          <div className="space-y-1">
                                            {face.topMatches.map((match, i) => (
                                              <div 
                                                key={i} 
                                          ...
                                          {face.topMatches[0] && face.topMatches[0].similarity >= 85 && (
                                            <div className="mt-2 p-2 bg-blue-50 border border-blue-200 rounded text-blue-800">
                                              <strong>💡 Sugerencia:</strong> Si este estudiante parece correcto, 
                    ...
                    {isSubmitting && uploadProgress > 0 && (
                      <div className="space-y-1">
                        ...
                        <p className="text-xs text-center text-muted-foreground">
                          {uploadProgress < 30 ? "Preparando fotos..." : 
                           uploadProgress < 70 ? "Subiendo fotos..." : 
              ...
              <CardContent>
                <div className="space-y-2 max-h-96 overflow-y-auto">
                  {classData.students.map((student) => (
                    <div key={student.student.id} className="p-3 bg-muted rounded-lg">
                      <div className="font-medium text-sm">{student.fullName}</div>
                      <div className="text-xs text-muted-foreground">
                        {student.faceCount > 0 ? (
                          <span className="text-green-600">✓ {student.faceCount} foto(s)</span>
                        ) : (
